...
// Ling 3.0 (bailing_hybrid). Same KDA + MLA hybrid shape as kimi-linear, with:
//  - single f_proj / g_proj instead of the low-rank pairs (config no_kda_lora)
//  - a lower bound on the KDA forget gate (config kda_safe_gate)
//  - a head-wise sigmoid gate on the MLA output
//  - RoPE on the MLA layers, which kimi-linear does not have
//  - group limited (noaux_tc) expert routing, as in bailingmoe2
...
void llama_model_bailing_hybrid::load_arch_hparams(llama_model_loader & ml) {
    ...
    // n_head_kv == 0 marks a KDA layer, the converter writes the pattern
    ...
    switch (hparams.n_layer()) {
        case 42: type = LLM_TYPE_A13B; break; // Ling-3.0-flash, 124B A5.1B
...
void llama_model_bailing_hybrid::load_arch_tensors(llama_model_loader &) {
    ...
    const int64_t d_kda      = hparams.n_embd_head_kda;
    const int64_t d_inner    = d_kda * n_head;
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (hparams.is_recr(i)) {
            // conv1d may be 3D when the trailing 1 is dropped by quantization
            ...
            layer.ssm_o_norm = create_tensor(tn(LLM_TENSOR_SSM_NORM, "weight", i), {d_kda}, 0);
            ...
            // q_lora_rank is null in this checkpoint, q is projected directly
...
// qkv selects which of the three conv states to use: 0 is Q, 1 is K, 2 is V
...
llama_model_bailing_hybrid::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    // reference scales by qk_head_dim, which is nope + rope
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (hparams.is_recr(il)) {
            ...
            // config rope_interleave is equivalent to the NORM rope layout here
            q_pe = ggml_rope_ext(ctx0, q_pe, inp_pos, nullptr,
                n_embd_head_qk_rope, rope_type, n_ctx_orig, freq_base, freq_scale, ext_factor, attn_factor, beta_fast, beta_slow);
            cb(q_pe, "q_pe", il);
            ...
            if (layer.wk_b && layer.wv_b) {
                ...
                cur = build_attn(inp_attn_k, nullptr, NULL, nullptr, Qcur, Kcur, Vcur, nullptr, nullptr, layer.wv_b, kq_scale_mla, il);
            } else {
                ...
                cur = build_attn(inp_attn_kv, nullptr, NULL, nullptr, Qcur, Kcur, Vcur, nullptr, nullptr, nullptr, kq_scale_mla, il);
            }

            // head-wise output gate, applied before the output projection
            ggml_tensor * gate = ggml_mul_mat(ctx0, layer.wqkv_gate, gate_inp);
            ...
            cur = ggml_cont_2d(ctx0, cur, n_embd_head_v_mla * n_head, n_tokens);
